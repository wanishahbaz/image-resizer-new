...
import { Download, ArrowRight, Settings2, FileDown } from 'lucide-react';
...
export default function PdfCompressor() {
  ...
  const processPdf = async () => {
    ...
    try {
      ...
      const targetImageBytes = (targetKB * 1024 * 0.95) / numPages; // distribute size evenly

      for (let pageNum = 1; pageNum <= numPages; pageNum++) {
        const page = await pdfDoc.getPage(pageNum);
        const viewport = page.getViewport({ scale: 2.0 }); // Render at 2x for better quality before compression
            ...
            <div className="flex-1 flex flex-col items-center justify-center bg-slate-50 rounded-xl border border-slate-200 p-8 mb-6 text-center">
              {compressedBlob ? (
                <div className="space-y-4">
                  ...
                  <h3 className="text-xl font-bold text-slate-800">Compression Complete!</h3>
